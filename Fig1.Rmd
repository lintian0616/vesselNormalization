...
## Visualization of GPAGs and PPAGs Using Heatmap
...
## Survival Analysis on Two Clusters in the Heatmap
...
## Test the ($\sum$GPAGs - $\sum$PPAGs) in METABRIC Validation Dataset
...
## Test the ($\sum$GPAGs - $\sum$PPAGs) for Relapse in (GSE1456)

```{r}
load("data/gse1456.RData") ## Used processed GSE1456.RData
```
...
## Test the ($\sum$GPAGs - $\sum$PPAGs) for Distant Metastasis in (GSE2990)

```{r}
load("data/gse2990.RData")
```
...
## Correlation of GPAGs and PPAGs with T Cell Activation
...
```{r}
library(LSD)
load("data/GO0042110_TcellActivation.RData")
